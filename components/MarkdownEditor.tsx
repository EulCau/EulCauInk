...
const handleCheckboxChange = (lineIndex: number) => {
      if (readOnly) {
          console.log("Read only mode, ignoring toggle");
          return;
      }
      
      const lines = content.split('\n');
      if (lineIndex < 0 || lineIndex >= lines.length) {
          console.error(`Line index ${lineIndex} out of bounds (Total: ${lines.length})`);
          return;
      }
      ...
      if (match) {
          ...
          // Fallback replacement if regex misses (e.g. malformed spaces)
          let newLine = line;
        ...
        >
          <ReactMarkdown
            ...
            components={{
              ...
              // Handle LI: Use position data to identify task lists and render custom interactive checkbox
              li: ({node, children, ...props}) => {
                  const lineIndex = node?.position?.start?.line ? node.position.start.line - 1 : -1;
                  
                  // Check if this LI is a task item by inspecting the raw content line
                  let isTask = false;
                  let isChecked = false;
                  ...
                  if (isTask) {
                      return (
                          <li className={`flex items-start my-1 ${props.className || ''}`}>
                              <input 
                                  type="checkbox" 
                                  checked={isChecked}
                                  onChange={(e) => {
                                      e.stopPropagation();
                                      handleCheckboxChange(lineIndex);
                                  }}
                                  disabled={readOnly}
                                  className="w-4 h-4 mt-1.5 mr-2 flex-shrink-0 cursor-pointer accent-blue-600"
                              />
              ...
              // Suppress default inputs generated by remark-gfm since we handle them in 'li'
              input: (props) => {
